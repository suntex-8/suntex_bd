...
import { useRef, useEffect, useState, useCallback, createElement, type CSSProperties, type KeyboardEvent, type MouseEvent } from "react";
    ...
    >
      {items.map((item, i) => {
        ...
        return createElement(
          Tag,
          panelProps,
          <span className="absolute inset-0 overflow-hidden [border-radius:inherit]" key="media">
            <span
              ref={(el: HTMLElement | null) => {
          ...
          showLabels && (
            <span className="pointer-events-none absolute bottom-5 left-5 right-5 z-[2] flex items-center gap-3" key="labels" aria-hidden="true">
              <span
                ref={(el: HTMLElement | null) => {
